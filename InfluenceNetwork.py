...
app = Flask(__name__)
CORS(app)
socketio = SocketIO(app, cors_allowed_origins="*", max_http_buffer_size=15000000, always_connect=True, ping_timeout=1200, async_mode='threading')
...
def build_influence_networks_with_stances(stances, index_user, users_tweet_text, mentions_matrix, mentions_matrix_date,
    ...
    socketio.emit("influence_heuristic",
                  ...
                  to=sid)
    def affinityEmit(event, val):
        socketio.emit(event, val, to=sid)
    affinities_matrix = build_affinities_matrix(users_tweet_text, stances, index_user, mentions_matrix_nonNorm, affinityEmit)
...
def calc_belif(users_tweet_text: ndarray[Set[str]], users: List[str], sid) -> Dict[str, float | None]:
...
def calculate_heuristics(users, df, sid):
    ...
    stances = calc_belif(users_tweet_text, users, sid)


    build_influence_networks_with_stances(stances, index_user, users_tweet_text, mentions_matrix, mentions_matrix_date,
                                          get_links_matrix, mentions_matrix_nonNorm, sid)
